...
class HeadlessCompatibilityTest {
  ...
  private async initHeadlessBrowser(): Promise<void> {
  ...
  private async testLoginPageAccess(): Promise<void> {
  ...
  private async testBasicFormInteraction(): Promise<void> {
    ...
    // 验证值是否正确填入
    const companyValue = await this.page.$eval('input[name="companyCode"]', el => (el as HTMLInputElement).value);
    const employeeValue = await this.page.$eval('input[name="employeeNo"]', el => (el as HTMLInputElement).value);
    const passwordValue = await this.page.$eval('input[name="password"]', el => (el as HTMLInputElement).value);
...
export { HeadlessCompatibilityTest };
